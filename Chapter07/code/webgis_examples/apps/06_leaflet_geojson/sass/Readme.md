...
folder are mapped to the application's root namespace, 'LeafletGeoJSON'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "LeafletGeoJSON"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'LeafletGeoJSON' in the same way
as `"LeafletGeoJSON/sass/src"`.
