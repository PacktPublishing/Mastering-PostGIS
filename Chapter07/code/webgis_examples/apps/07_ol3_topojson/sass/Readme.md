...
folder are mapped to the application's root namespace, 'Ol3TopoJson'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "Ol3TopoJson"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'Ol3TopoJson' in the same way
as `"Ol3TopoJson/sass/src"`.
