...
folder are mapped to the application's root namespace, 'Ol3GeoJSON'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "Ol3GeoJSON"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'Ol3GeoJSON' in the same way
as `"Ol3GeoJSON/sass/src"`.
