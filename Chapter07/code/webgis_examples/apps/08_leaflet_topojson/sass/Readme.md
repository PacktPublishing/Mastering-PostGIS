...
folder are mapped to the application's root namespace, 'LeafletTopoJson'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "LeafletTopoJson"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'LeafletTopoJson' in the same way
as `"LeafletTopoJson/sass/src"`.
