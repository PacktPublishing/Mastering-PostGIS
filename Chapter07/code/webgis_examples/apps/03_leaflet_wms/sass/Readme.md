...
folder are mapped to the application's root namespace, 'LeafletWms'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "LeafletWms"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'LeafletWms' in the same way
as `"LeafletWms/sass/src"`.
